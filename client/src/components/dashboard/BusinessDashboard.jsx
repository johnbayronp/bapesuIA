...
export default function BusinessDashboard() {
  ...
  const { user, profile, company, loading } = useCompany();
        ...
        {BIZ.map((item) => {
          ...
          return (
            <Link
              ...
              onClick={() => setSidebarOpen(false)}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 ${
                active ? item.activeBg : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
              }`}
            >
              <span className={active ? '' : item.accent}>{item.icon}</span>
              {item.label}
            </Link>
          );
        })}
